export type BudgetLimits = {
  ...
  requireKnownSubscriptionLimits?: boolean;
};
...
export function estimateRunBudget(
  ...
  const maxOutputCharsPerCandidate =
    input.maxOutputChars ?? DEFAULT_COMPARE_MAX_OUTPUT_CHARS;
  ...
  return {
    ...
    assumptions: [
      ...
      "Compare estimates assume every selected task may contribute up to maxOutputChars.",
...
export function estimateCompareBudget(
  ...
  return {
    ...
    assumptions: [
      ...
      "Task candidates are estimated at maxOutputChars; direct response candidates use their provided text length.",
      "Output tokens, tool calls, cache behavior, and provider pricing are not known before a run starts.",
      "Subscription quota checks use supported CLI/API surfaces only; Ennodia does not inspect private account pages or provider-private APIs.",
    ],
    subscriptionLimitChecks: selectedHarnessIds.map(subscriptionLimitCheck),
...
export function checkBudgetLimits(
  ...
  if (
    limits?.requireKnownSubscriptionLimits &&
    estimate.subscriptionLimitChecks.some((check) => check.status !== "known")
  ) {
    issues.push(
      "Subscription limits are not known for every selected harness through supported local surfaces.",
    );
  }
